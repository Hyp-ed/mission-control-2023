...
(function() {
  ...
  History = class History {
    ...
    connect(callback) {
      ...
      return this.client.getDatabaseNames().then((names) => {
    ...
    prepareSchema() {
      var dictionary, i, k, len, ref, ref1, schema, table, val;
      schema = [];
    ...
    recordBatch(points, callback) {
      ...
      return this.client.writePoints(pts).then(function() {
        return callback();
      }).catch(function(e) {
        return callback(e);
      });
    ...
    query(id, start, end, callback) {
      var endString, measurement, query, startString;
      if (!this.client) {
        return callback(new Error('Not connected to InfluxDB'));
      }
      ...
      query = `select value from ${this.prepareId(id)} where time > '${startString}' and time < '${endString}' order by time asc;`;
      return this.client.query(query).then(function(result) {
        var points;
